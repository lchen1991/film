...
import com.filmresource.cn.bean.CardItemView;
...
public class CardSlidePanel extends ViewGroup {
    ...
    private static final int[] layerDrawables = new int[] {
            R.drawable.transparent_shade_layer,
            R.drawable.first_shade_layer,
            R.drawable.second_shade_layer,
    ...
    private List<FilmInfo> dataList; // 存储的数据链表
    ...
    private void orderViewStack() {
        ...
        synchronized (obj1) {
            ...
            if (isCyclic) {
                FilmInfo dataItem = dataList.get(newIndex % dataList.size());
                changedView.fillData(dataItem);
                isShowing++;
            } else {
                if (newIndex < dataList.size()) {
                    FilmInfo dataItem = dataList.get(newIndex);
                    changedView.fillData(dataItem);
                } else {
     ...
     * @param dataList 数据
     */
    public void fillData(List<FilmInfo> dataList) {
     ...
     */
    public void appendData(List<FilmInfo> appendList) {
